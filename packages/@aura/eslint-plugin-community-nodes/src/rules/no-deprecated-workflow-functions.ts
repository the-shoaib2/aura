...
export const NoDeprecatedWorkflowFunctionsRule = createRule({
	...
	meta: {
		type: 'problem',
		docs: {
			description: 'Disallow usage of deprecated functions and types from workflow package',
		},
		messages: {
	...
	create(context) {
		...
		return {
			...
			ImportSpecifier(node) {
				// Check if this import is from workflow by looking at the parent ImportDeclaration
				const importDeclaration = node.parent;
				if (
					importDeclaration?.type === AST_NODE_TYPES.ImportDeclaration &&
					importDeclaration.source.value === 'workflow' &&
