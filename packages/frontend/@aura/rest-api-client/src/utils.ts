...
import type { GenericValue, IDataObject } from 'workflow';
...
export async function request(config: {
	...
	if (
		...
		!baseURL.includes('api.aura.io') &&
		!baseURL.includes('aura.cloud')
	) {
	...
	try {
		...
		if (error.message === 'Network Error') {
			throw new ResponseError("Can't connect to aura.", {
