...
export function getConfigFromMetaTag(configName: string): string | null {
	...
	try {
		...
		console.warn(`Failed to read aura config for "${tagName}":`, error);
		return null;
	}
}
...
export function getAndParseConfigFromMetaTag<T>(configName: string): T | null {
	...
	try {
		...
		console.warn(`Failed to parse aura config for "${getTagName(configName)}":`, error);
		return null;
	}
}
