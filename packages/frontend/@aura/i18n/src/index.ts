...
import type { INodeProperties, INodePropertyCollection, INodePropertyOptions } from 'workflow';
import { ref } from 'vue';
...
export class I18nClass {
	...
	shortNodeType(longNodeType: string) {
		return longNodeType.replace('aura-nodes-base.', '');
	}
	...
	credText(credentialType: string | null) {
		const credentialPrefix = `aura-nodes-base.credentials.${credentialType}`;
		const context = this;
	...
	nodeText(activeNodeType?: string | null) {
		...
		const initialKey = `aura-nodes-base.nodes.${shortNodeType}.nodeView`;
		const context = this;
		...
		return {
			...
			eventTriggerDescription(nodeType: string, eventTriggerDescription: string) {
				return context.dynamicRender({
					key: `aura-nodes-base.nodes.${nodeType}.nodeView.eventTriggerDescription`,
