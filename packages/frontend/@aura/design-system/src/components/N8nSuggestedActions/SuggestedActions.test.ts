...
const mockActions = [
	{
		...
		moreInfoLink: 'https://docs.aura.io/evaluations',
	},
	{
		id: 'action2',
...
const stubs = {
	...
	N8nCallout: {
		props: ['theme'],
		template: '<div data-test-id="aura-callout" :class="theme"><slot /></div>',
	},
...
describe('N8nSuggestedActions', () => {
	...
	it('renders more info link when moreInfoLink is provided', async () => {
		...
		expect(link.getAttribute('href')).toBe('https://docs.aura.io/evaluations');
	});
	...
	describe('Notice functionality', () => {
		it('renders notice alert when notice prop is provided', async () => {
			...
			const callout = wrapper.getByTestId('aura-callout');
		...
		it('does not render notice callout when notice prop is not provided', async () => {
			...
			expect(wrapper.queryByTestId('aura-callout')).not.toBeInTheDocument();
		});
